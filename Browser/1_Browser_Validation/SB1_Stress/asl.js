/* This asl library is based on Shell version 2.8.6 */
...
window.onbeforeunload = function(e){
    if(unload == 0){
        asl.prv.message(asl.msgs.types.pageChange, {id:asl.prv.id, url: window.location.href});
        unload = 1;
    }
};
            ...
            if(object[params.data.method].indexOf('http://') != 0) {
                ...
                if(url_param.length == 2){
                    ...
                    for(var i=0; i<p.length; i++){
                        var param = p[i].split('=');
                        var prop = param[0].substr(0, 1).toLowerCase() + param[0].substr(1);
                        param_values.push(param[0] + '=' + encodeURIComponent(params.data.data[prop])); 
                    }
                }
                ...
                if(url_params.length == 0){
                    for(var p in props){
                        param_values.push(props[p] + "=" +  encodeURIComponent(params.data.data[props[p]]));
                    }   
                }
                address = url_param[0] + '?' + (param_values ? param_values.join('&') : "");
...
asl.notify = function(type, priority, title, message, buttons, actions, timeout, iconPath) {
    ...
    for(var i = 0, length = actions.length; i < length; i++) {
        if(actions[i]){
            if(actions[i].toString().indexOf("function") == 0) params.actions.push(asl.fn.store.stringify(eval(actions[i])));
            else params.actions.push(asl.fn.store.stringify(actions[i]));
        }
        else{
 ...
 */
asl.keyboard = function(type, callback, text, title, maxlength, mode){
    var params = {};
    asl.kbdOn = true;
    ...
    params.mode = type instanceof HTMLInputElement && type.getAttribute('data-mode') ? type.getAttribute('data-mode') : "";
