...
(function(c) {
	function C(n, q) {
		...
		function t() {
			...
			if (!b.displayOnly)
				if (b.lineWidth) {
					...
					h.moveTo(b.lineMargin, 0);
					h.lineTo(b.lineMargin, k.height);
					h.stroke();
					h.closePath()
				}
		...
		function z(a) {
			for (c(b.typed, d).html(
					a.replace(/>/g, "&gt;").replace(/</g, "&lt;")); c(b.typed,
					d).width() > k.width;) {
		...
		function F(a, e, f) {
			if (a.nameInvalid) {
				e.prepend([ '<p class="', f.errorClass, '">', f.errorMessage,
						"</p>" ].join(""));
				c(f.name, e).focus();
				c(f.name, e).addClass(f.errorClass);
				c("label[for=" + c(f.name).attr("id") + "]", e).addClass(
						f.errorClass)
			}
			a.drawInvalid
					&& e.prepend([ '<p class="', f.errorClass, '">',
							f.errorMessageDraw, "</p>" ].join(""))
		}
		function A() {
			var a = true, e = {
				drawInvalid : false,
			...
			b.onBeforeValidate && typeof b.onBeforeValidate === "function" ? b.onBeforeValidate
					.apply(p, f)
					: E.apply(p, f);
			...
			b.onFormError && typeof b.onFormError === "function" ? b.onFormError
					.apply(p, g)
					: F.apply(p, g);
			return a
		}
		...
		function G() {
			if (parseFloat((/CPU.+OS ([0-9_]{3}).*AppleWebkit.*Mobile/i
					.exec(navigator.userAgent) || [ 0, "4_2" ])[1].replace("_",
					".")) < 4.1) {
			...
			if (k.getContext) {
				...
				if (!b.displayOnly) {
					...
					if (b.validateFields)
						c(n).is("form") ? c(n).bind("submit.signaturepad",
								function() {
									return A()
								}) : c(n).parents("form").bind(
		...
		var p = this, b = c.extend({}, c.fn.signaturePad.defaults, q), d = c(n), i = c(
				b.canvas, d), k = i.get(0), h = null, j = {
			x : null,
			y : null
		}, l = [], o = false, r = false, u = false;
		c.extend(p, {
			init : function() {
				G()
			},
			regenerate : function(a) {
				...
				if (typeof a === "string")
					a = JSON.parse(a);
				B(a, h, true);
				c(b.output, d).length > 0
						&& c(b.output, d).val(JSON.stringify(l))
			},
