...
function SignaturePad (selector, options) {
   ...
   * {
   *   d: moveTo x coordinate, moveTo y coordinate, lineTo x coordinate, lineTo y coordinate
  ...
  function drawLine (e, newYOffset) {
    ...
    // we can avoid Math.floor as we only have small positive numbers to deal with
    //if (typeof e.changedTouches !== 'undefined') {
    if (touchable) {    	
      //newX = Math.floor(e.changedTouches[0].pageX - offset.left)
      newX = ~~(e.changedTouches[0].pageX - offset.left);
      //newY = Math.floor(e.changedTouches[0].pageY - offset.top)
      newY = ~~(e.changedTouches[0].pageY - offset.top);     
    } else {
      clearTimeout(mouseLeaveTimeout)
      mouseLeaveTimeout = false
      
      //newX = Math.floor(e.pageX - offset.left)
      newX = ~~(e.pageX - offset.left);
      //newY = Math.floor(e.pageY - offset.top)
      newY = ~~(e.pageY - offset.top);      
    }
    ...
    //consoleLog(newX+", "+newY+", "+newYOffset+", "+previous.x+", "+previous.y);
	...
	*/
    output.push({
      'd': newX+","+newY+","+previous.x+","+previous.y
    })
  ...
  function drawSigLine () {
    ...
    canvasContext.lineTo(padWidth() - settings.lineMargin, settings.lineTop)
    canvasContext.stroke()
    canvasContext.closePath()
  }

  /**
   * Returns a width band based on the minimum viewport width 
   */
  function padWidth() {
	  // find min direction
	  if ($(window).width() > $(window).height()) // landscape
	  {
		  sigWidth = $(window).height();
	  }
	  else
	  {
		  sigWidth = $(window).width();
	  }	  
	  // select width from band - these may need to be tuned for different devices
	  var sizeBand = new Array(0,280,401,501,701,1000,1500,3000);
	  var widthBand = new Array(200,275,350,500,700,800,800);
	  var width = 200;
	  for (x=0;x<sizeBand.length-1;x++)
	  {
		  if (sigWidth >= sizeBand[x] && sigWidth < sizeBand[x+1])
		  {
			  width = widthBand[x];
			  break;
		  }
	  ...
	  // update the styles
  	  $('.sigpad').css('width', width);
  	  $('.sigwrapper').css('width', width);  	
  	  $('.pad').css('width', width);  
  	  $('.pad').css('height', 150);
  	  //$('#dev').text("Sigpad width: "+sigWidth+", "+width); // display dev info on signature page
  	  return width - 2;
  }
  ...
  function clearCanvas () {
    ...
    element.width = padWidth()
    canvasContext.fillStyle = settings.bgColour
    canvasContext.fillRect(0, 0, padWidth(), element.height)
  ...
  function type (val) {
    ...
    while ($(settings.typed, context).width() > padWidth()) {
  ...
  function drawSignature (paths, context, saveOutput) {
    ...
    for(var i in paths) {
      if (typeof paths[i] === 'object') {
        ...
        context.moveTo(dArray[0],dArray[1]);
        context.lineTo(dArray[2],dArray[3]);        
   ...
   */
  function init () {
	// change width to surrounding pad size
	element.width = padWidth();
  ...
  $.extend(self, {
    ...
    , getSignatureImage: function () {
      ...
      tmpContext.fillRect(0, 0, padWidth(), element.height)
...
$.fn.signaturePad.defaults = {
  ...
  , penWidth: 1 // Thickness of the pen
  , penCap: 'square' // Determines how the end points of each line are drawn (values: 'butt', 'round', 'square')
  , lineColour: '#ddd' // Colour of the signature line
