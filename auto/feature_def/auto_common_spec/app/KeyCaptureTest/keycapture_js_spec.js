...
*/			
	...
	it("2.12.should obstruct the key event from reaching the page when dispatch is set to false", function()
	{
		runs(function()
		{
			hardwareKeysTest.captureKey(false,'' + hardwareKeysTest.LETTER_A_CODE);
			hardwareKeysTest.createTextBox();
			hardwareKeysTest.textBox.focus();
			hardwareKeysTest.simulateKeyPress(hardwareKeysTest.LETTER_A_CODE);
			setTimeout(function(){hardwareKeysTest.callbackFiredTimeout = 'timedout'},1000);// TAB
...
/*	if (isWindowsMobilePlatform())
	{
		it("VT289-039 | call remapKey with enter and numeric key 1 |", function()
		{
			runs(function()
			{
				hardwareKeysTest.remapKey('0x0D','0x31');
				hardwareKeysTest.createTextBox();
				hardwareKeysTest.textBox.focus();
				hardwareKeysTest.simulateKeyPress(0x0D);
				hardwareKeysTest.simulateKeyPress(0x31);
				setTimeout(function(){hardwareKeysTest.callbackFiredTimeout = 'timedout'},1000);// TAB
			});
			...
			runs(function()
			{		
				expect(hardwareKeysTest.textBox.value).toEqual('11');
				
				//Tear down
				hardwareKeysTest.clearRemap('0x0D');
				hardwareKeysTest.removeTextBox();
				hardwareKeysTest.callbackFiredTimeout = null;
			});
		});
		...
		it("VT289-040 | call remapKey with functionkey F1 and numeric key 9 |", function()
		{
			runs(function()
			{
				hardwareKeysTest.remapKey('0x70','0x39');
				hardwareKeysTest.createTextBox();
				hardwareKeysTest.textBox.focus();
				hardwareKeysTest.simulateKeyPress(0x70);
				hardwareKeysTest.simulateKeyPress(0x39);
				setTimeout(function(){hardwareKeysTest.callbackFiredTimeout = 'timedout'},1000);// TAB
			});
			...
			runs(function()
			{		
				expect(hardwareKeysTest.textBox.value).toEqual('99');
				
				//Tear down
				hardwareKeysTest.clearRemap('0x70');
				hardwareKeysTest.removeTextBox();
				hardwareKeysTest.callbackFiredTimeout = null;
			});
		});
		...
		it("VT289-044 | call capturekey after remapKey |", function() {
			...
			runs(function()
			{
				...
				Rho.KeyCapture.remapKey('0x41','0x42');
				Rho.KeyCapture.captureKey(true,'0x41',capturekeycallback);
			});
			...
			{		
				...
				runs(function() {
				expect("pass").toEqual(document.getElementById("actResult").innerHTML);
				Rho.KeyCapture.remapKey('0x41');
				Rho.KeyCapture.captureKey(true,'0x41');
				});	
			});
		});
