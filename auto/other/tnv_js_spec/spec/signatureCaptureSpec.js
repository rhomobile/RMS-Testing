...
function JSONSigtranseferEvent(jsonobject)
{
	eventOutput = "";
...
function Jsvectoryarray(vectorArray)
{
  ...
  {
    eventOutput = eventOutput + "(" + vectorArray[i] + "," + vectorArray[i+1] + ") <br/>";
  }
	eventFired  = true ;
}

function Jsonvectoryarray(jsondata)
{
	eventFired = false;
	eventOutput = "";

  for (var i=0; i<jsondata.vectorArray.length; i = i + 2)
  {
  	eventOutput = eventOutput + "(" + jsondata.vectorArray[i] + "," + jsondata.vectorArray[i+1] + ") <br/>";
