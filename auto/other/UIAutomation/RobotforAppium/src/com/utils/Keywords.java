...
public class Keywords {
	 ...
	 * @return
	 */
	public String SelectFromDropdown(Hashtable<String,String> getvalue,String objname){
		log("Entering the SelectFromDropdown function");
		int count=0;
		try{
			String[] args = null;
			args = objname.split(",");
			//WebElement temp1 = element(args[1]);		
			 WebElement dropdown = element(args[0]);
			 Select select = new Select(dropdown);
			 String selecteditem = null;
			 //Get VTID from OR.properties to compare with full testspec description 
			 //String obj=OR.getProperty(objname);
			 //Get full testspec description and slect the perticular test
			 List<WebElement> options = select.getOptions();
			    for (WebElement elementweb : options) {
			            if(elementweb.getText().contains(args[1])) {
			            	count=1;
			            	selecteditem = elementweb.getText();
			            	select.selectByVisibleText(selecteditem);
			        }
			...
			Thread.sleep(1000);
			if(count==1){
				log("Successfull in selecting string from SelectFromDropdown");
				return "pass";
			}
			else{
				log("Not able to select from SelectFromDropdown");
				return "fail";
			}
		}
		catch(Exception e){
			log("Exiting the SelectFromDropdown function"+e.getMessage());
			e.printStackTrace();
			return "fail";
		}
	}	
