...
describe('EB.PrinterZebra Search Printer JS API', function () {
    var printers = [];
    ...
    function searchPrinterCallback(callbackValue) {
        ...
        if ((callbackValue.status == EB.PrinterZebra.PRINTER_STATUS_SUCCESS) && callbackValue.printerID && (callbackValue.printerID.length > 0)) {
            ...
            if (callbackValue.status == EB.PrinterZebra.PRINTER_STATUS_ERR_TIMEOUT) {
                alert("Timeout");
            }
    ...
    describe('EB.PrinterZebra APIs Set Test', function () {
        it("Search Printer", function () {
            runs(function () {
                EB.PrinterZebra.searchPrinters({"printerType": EB.PrinterZebra.PRINTER_TYPE_ZEBRA, "connectionType": EB.PrinterZebra.CONNECTION_TYPE_TCP, "timeout": 30000}, searchPrinterCallback);
            });
            waitsFor(function () {
                if (searched)
                    return true;
        ...
        describe("Printer type " + JSON.stringify(printer_instance), function () {
            describe('Connect to Printer', function () {
                it('Should connect to a printer and then start tests', function () {
                    runs(function () {
                        printer_instance = EB.PrinterZebra.getPrinterByID(printers[0]);
            ...
            if (printer_instance.connectionType != EB.PrinterZebra.CONNECTION_TYPE_BLUETOOTH) {
