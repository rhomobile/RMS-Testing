...
hardwareKeysTest.deviceOS = EB.System.platform;
...
{
	EB.Instrumentation.simulate_key_event_code(keyValue);
};
...
{
	EB.KeyCapture.captureKey(bDispatch,key,hardwareKeysTest.captureKeyCallback);
};

hardwareKeysTest.resetCaptureKeyCallback = function(key)
{
	EB.KeyCapture.captureKey(true,key); //NULL callback
};
...
{
	EB.KeyCapture.remapKey(keyFrom, keyTo); 
};

hardwareKeysTest.clearRemap = function(keyFrom)
{
	EB.KeyCapture.remapKey(keyFrom, null); 
};
...
{
	EB.KeyCapture.captureTrigger(hardwareKeysTest.triggerCallback);
};
