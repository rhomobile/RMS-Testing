describe("Intent JS API Test", function() {
	...
	for (var j=0; j<paramArray.length; j++){
		...
		if(isAndroidPlatform()){
			it(testCase[j].id +" | "+testCase[j].description, function() {
				runs(function(){
					EB.Intent.send(paramArray[i], callbackgetproperties);
				});
				waitsFor(function(){
	...
	it('VT328_58 | appName - Try to Launch non-existing Application via \'appName\' from test application.', function () {
        var parameters = {intentType: EB.Intent.START_ACTIVITY, action: 'android.intent.action.MAIN', appName: 'nonExistingApp'}
        expect(function () {
	...
	if(isAndroidPlatform()){
	    it('VT328_59 | intentType - StartActivity: Try to launch target appilcation by \'className\', which is not installed.', function () {
		    var parameters = {intentType: EB.Intent.START_ACTIVITY, action: 'android.intent.action.MAIN', appName: 'com.notInstalled', targetClass: 'dummyClass'}
		    expect(function () {
		        EB.Intent.send(parameters)
		    }).toThrow();
	    });
		it('VT328_60 | Start activity of absent application should raise exception', function () {
		    var parameters = {intentType: EB.Intent.START_ACTIVITY, action: 'android.intent.action.MAIN', appName: 'com.notInstalled'}
		    expect(function () {
		        EB.Intent.send(parameters)
		    }).toThrow();
		});
    ...
    it('VT328_63 | Sending Intent with callback which does\'n handle the input argument shouldn\'t raise error', function(){
        var parameters = {
            ...
            data: null}
        expect(function () {
            EB.Intent.send(parameters, function(){})
        }).not.toThrow();

    });
    it('VT328_64 | Sending Intent with variable in place of callback function', function(){
        var parameters = { intentType: EB.Intent.START_ACTIVITY, action: 'android.intent.action.MAIN', appName: 'com.smap.targetapp' };
        var successCB = "This is not a call back function!";
        expect(function () {
            EB.Intent.send(parameters, successCB);
        }).toThrow();
    });
    if(isAndroidPlatform()){
	    it('VT328_65 | Broad cast with callback', function () {
	        ...
	        var params = new parameters(EB.Intent.BROADCAST,"","com.rhomobile.BROADCAST",["com.rhomobile.manual_common_spec"],"","","","",data);
	        ...
	        expect(function () {
	        	EB.Intent.send(params, receiveCB);
	        }).not.toThrow();
		});
    }
    it('VT328_66 | Start Listening to the background intents with empty callback', function () {
        ...
        if (isApplePlatform()) {
            parameters = {intentType: EB.Intent.BROADCAST, action: "com.rhomobile.BROADCAST", appName: "testapp", data: {myData: "This is broad cast data!" } };
        }
        if (isAnyWindowsFamilyPlatform()) {
            parameters = {intentType: EB.Intent.BROADCAST, appName: "rhomobile TestApp/TestApp.exe", data: {myData: "This is broad cast data!" } };
        }
    ...
    if ( isAndroidPlatform() ) {
        it('VT328_66 | Pass intent data array with unsupported type', function() {
            var parameters = { intentType:EB.Intent.START_ACTIVITY, action:"ACTION_SEND", data:{"EXTRA_EMAIL":[0.1,0.2,0.3]} };
