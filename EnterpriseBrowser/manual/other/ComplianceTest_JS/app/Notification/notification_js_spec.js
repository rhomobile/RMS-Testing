describe("Notification Manual FD Tests", function () {
	...
	if ((isWindowsMobilePlatform() || isAndroidPlatform()) && !EB.System.isRhoSimulator) 
	{
		...
		it("VT200-0653|Play File - Mp3 file with media type|", function () {
			...
			alert(EB.RhoFile.join('/sdcard/Application/', 'media1.mp3'), '.mp3');
			EB.Notification.playFile(EB.RhoFile.join('/sdcard/Application/', 'media1.mp3'), '.mp3');
			}else{
			EB.Notification.playFile(EB.RhoFile.join('/Application/', 'media2.wav'), '.wav');
			}
			
			//	EB.Notification.playFile(EB.RhoFile.join('/Application/', 'media1.mp3'), '.mp3');
