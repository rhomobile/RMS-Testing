...
function updatePrinterList(printers) {
    for (var i = 0; i < printers.length; i++) {
        var printerInstance = EB.Printer.getPrinterByID(printers[i]);
...
function runSearch(options, timeout) {
    ...
    function searchPrinterCallback(callbackValue) {
        ...
        if (callbackValue.status == EB.Printer.PRINTER_STATUS_SUCCESS) {
            if (printer_id && printer_id.length > 0) {
                SO.discovered.push(printer_id);
                SO.last_printer_id = printer_id;
                SO.last_printer = EB.Printer.getPrinterByID(printer_id);
            } else {
                checkSearch();
            }
        } else {
    ...
    function checkSearch() {
        ...
        if ((SO.curr > SO.total) || ((elapsed + last_el) > timeout)) {
            ...
            if (SO.curr <= 1) {
                EB.Printer.searchPrinters(options, searchPrinterCallback);
            } else {
                setTimeout(function() {
                    EB.Printer.searchPrinters(options, searchPrinterCallback);
                }, 200);
...
$(window).load(
    function() {
        var conn_types = [EB.Printer.CONNECTION_TYPE_BLUETOOTH, EB.Printer.CONNECTION_TYPE_ON_BOARD, EB.Printer.CONNECTION_TYPE_TCP];
