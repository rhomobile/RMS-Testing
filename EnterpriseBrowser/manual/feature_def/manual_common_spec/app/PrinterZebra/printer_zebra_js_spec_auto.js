describe('Printer Zebra', function() {
    ...
    function doConnect() {
        ...
        runs(function() {
            ...
            if (last_found_printer_id == null) {
                var allprinters = EB.PrinterZebra.enumerate();
    ...
    function doPrintPrintFile(filename, options, isOk) {
        ...
        runs(function() {
            ...
            if (isOk !== false) {
                ...
                expect(callresult).toNotEqual(EB.PrinterZebra.PRINTER_STATUS_SUCCESS);
            }
        });
    }
    ...
    it('initialize before tests', function() {
        ...
        runs(function() {
            ...
            window.onunload = function(){
                var printer = EB.PrinterZebra.getPrinterByID(last_found_printer_id);
                printer.disconnect();
            };
    ...
    describe('enumerateSupportedTypes method', function() {
        it('using result', function() {
            runs(function() {
                ...
                var printerTypes = EB.PrinterZebra.enumerateSupportedTypes();
                expect(printerTypes).toEqual([EB.PrinterZebra.PRINTER_TYPE_ZEBRA]);
            });
        });
        it('using callback', function() {
            ...
            runs(function() {
                EB.PrinterZebra.enumerateSupportedTypes(enumerateCallback);
            });
            waitsFor(function() {
                return enumCb !== null;
            }, 'Timed out waiting for testing callback', 2000);
            runs(function() {
                expect(enumCb).toEqual([EB.PrinterZebra.PRINTER_TYPE_ZEBRA]);
            });
        });
    ...
    describe('should getPrinterByID', function() {
        ...
        it('and get base properties', function() {
            ...
            runs(function() {
                ...
                var thisprinter = EB.PrinterZebra.getPrinterByID(last_found_printer_id);
    ...
    // connect printers
    xdescribe('connect and disconnect methods', function() {
        var callresult = null;
        ...
        it('disconnect and try to print should return status error', function() {
           ...
           var callresultp = null;
            ...
            runs(function() {
                expect(callresultp.status).toBeIn([EB.PrinterZebra.PRINTER_STATUS_ERROR,EB.PrinterZebra.PRINTER_STATUS_ERR_NOT_CONNECTED]);
            });
        });
            ...
            it('should connect ' + case_type + ' callback; params:' + JSON.stringify(connectparams, null, " "), function() {
                ...
                runs(function() {
                     if (expectedResult) {
                        ...
                        if (case_type != 'without') {
                            expect((callresult == EB.Printer.PRINTER_STATUS_ERR_TIMEOUT) || (callresult == EB.Printer.PRINTER_STATUS_ERROR)).toEqual(true);
                        }
                        expect(thisprinter.isConnected).toEqual(false);
                     }
                });
        ...
        //if (EB.System.platform != EB.System.PLATFORM_ANDROID) {
    ...
    xdescribe('retrieveFileNames method', function() {
    ...
    function generateStoreImage(from, to, width, height, isOk, force) {
        if ((!EB.RhoFile.exists(from)) || (width == 0) || (height == 0)) {
            if (!isOk && !force) {
                return;
            }
            isOk = false;
        ...
        var deftext = [def, 'store image', EB.RhoFile.basename(from), '=>', to, '[w:', width, 'h:', height, ']'];
    ...
    describe('enumerateSupportedControlLanguages method', function() {
        ...
        it('using result', function() {
            runs(function() {
                ...
                expect(languagesTypes).toEqual([EB.PrinterZebra.PRINTER_LANGUAGE_ZPL, EB.PrinterZebra.PRINTER_LANGUAGE_CPCL]);
            });
        });
        it('using callback', function() {
            var enumCb = null;
            ...
            runs(function() {
                expect(enumCb).toEqual([EB.PrinterZebra.PRINTER_LANGUAGE_ZPL, EB.PrinterZebra.PRINTER_LANGUAGE_CPCL]);
            });
        });
        it('using anonymous callback', function() {
            var enumCb = null;
    ...
    var listofrequeststate = [EB.PrinterZebra.PRINTER_STATE_IS_HEAD_COLD, EB.PrinterZebra.PRINTER_STATE_IS_HEAD_OPEN, EB.PrinterZebra.PRINTER_STATE_IS_HEAD_TOO_HOT, EB.PrinterZebra.PRINTER_STATE_IS_PARTIAL_FORMAT_IN_PROGRESS, EB.PrinterZebra.PRINTER_STATE_IS_PAUSED, EB.PrinterZebra.PRINTER_STATE_IS_RECEIVE_BUFFER_FULL, EB.PrinterZebra.PRINTER_STATE_IS_RIBBON_OUT, EB.PrinterZebra.PRINTER_STATE_LABEL_LENGTH_IN_DOTS, EB.PrinterZebra.PRINTER_STATE_LABELS_REMAINING_IN_BATCH, EB.PrinterZebra.PRINTER_STATE_NUMBER_OF_FORMATS_IN_RECEIVE_BUFFER, EB.PrinterZebra.PRINTER_STATE_PRINT_MODE, EB.PrinterZebra.PRINTER_STATE_IS_READY_TO_PRINT, EB.PrinterZebra.PRINTER_STATE_IS_COVER_OPENED, EB.PrinterZebra.PRINTER_STATE_IS_DRAWER_OPENED, EB.PrinterZebra.PRINTER_STATE_IS_PAPER_OUT, EB.PrinterZebra.PRINTER_STATE_IS_BATTERY_LOW];
    ...
    var printmode_values = [EB.PrinterZebra.PRINT_MODE_APPLICATOR, EB.PrinterZebra.PRINT_MODE_CUTTER, EB.PrinterZebra.PRINT_MODE_DELAYED_CUT, EB.PrinterZebra.PRINT_MODE_KIOSK, EB.PrinterZebra.PRINT_MODE_LINERLESS_PEEL, EB.PrinterZebra.PRINT_MODE_LINERLESS_REWIND, EB.PrinterZebra.PRINT_MODE_LINERLESS_REWIND, EB.PrinterZebra.PRINT_MODE_PARTIAL_CUTTER, EB.PrinterZebra.PRINT_MODE_PEEL_OFF, EB.PrinterZebra.PRINT_MODE_REWIND, EB.PrinterZebra.PRINT_MODE_RFID, EB.PrinterZebra.PRINT_MODE_TEAR_OFF, EB.PrinterZebra.PRINT_MODE_UNKNOWN];
    var requeststate_callbackValue = {};

    function requestStateCallback(args) {
        if (args.status == EB.PrinterZebra.PRINTER_STATUS_SUCCESS) {
            expect(args.status).toEqual(EB.PrinterZebra.PRINTER_STATUS_SUCCESS);
    ...
    describe('requestState method', function() {
        ...
        it("Result of Request printer state Boolean values", function() {
            runs(function() {
                expect(callresult.status).toEqual(EB.PrinterZebra.PRINTER_STATUS_SUCCESS);
                if (callresult.status == EB.PrinterZebra.PRINTER_STATUS_SUCCESS) {
    ...
    describe("Get & Set default PrinterZebra", function() {
        ...
        it('set default PrinterZebra', function() {
            runs(function() {
                printerObj = EB.PrinterZebra.getPrinterByID(last_found_printer_id);
                EB.PrinterZebra.setDefault( printerObj );
            });
        });

        it('get default PrinterZebra', function() {
            runs(function() {
                expect(EB.PrinterZebra.ID).toNotEqual(null);
                expect(EB.PrinterZebra.ID).toEqual(last_found_printer_id);
                expect(EB.PrinterZebra.getDefault().ID).toEqual(printerObj.ID);
            });
        });
    });
});
