...
class SettingsController < Rho::RhoController
  ...
  def index
    @msg = @params['msg']
    $cnt = 0
    Rho::Dummy.enable(2, url_for(:action => :myEnableCallback))
    render 
  end
  def myEnableCallback
    puts "HASHMAP #{@params.inspect}"
    $cnt = $cnt + 1
    data = $cnt
    if @params["status"] == "ok"  
        Rho::WebView.executeJavascript("showCounter('" + data.to_s + "');")
    end
  end
  ...
  def indexstr
     @msg = @params['msg']
     $cnt = 0
     Rho::Dummy.enableString(2, url_for(:action => :myEnableCallbackStr))
     render 
   end
   def myEnableCallbackStr
     puts "STRING #{@params.inspect}"
     $cnt = $cnt + 1
     data = $cnt
       
         Rho::WebView.executeJavascript("showCounter2('" + data.to_s + "');")
  ...
  def indexint
     @msg = @params['msg']
     $cnt = 0
     Rho::Dummy.enableInt(2, url_for(:action => :myEnableCallbackInt))
     render 
   end
   def myEnableCallbackInt
     puts "INTEGER #{@params.inspect}"
     $cnt = $cnt + 1
     data = $cnt
       
         Rho::WebView.executeJavascript("showCounter3('" + data.to_s + "');")
  ...
  def indexfloat
     @msg = @params['msg']
     $cnt = 0
     Rho::Dummy.enableFloat(2, url_for(:action => :myEnableCallbackFloat))
     render 
   end
   def myEnableCallbackFloat
     puts "FLOAT #{@params.inspect}"
     $cnt = $cnt + 1
     data = $cnt
       
         Rho::WebView.executeJavascript("showCounter4('" + data.to_s + "');")
  ...
  def indexboolean
     @msg = @params['msg']
     $cnt = 0
     Rho::Dummy.enableBool(2, url_for(:action => :myEnableCallbackBool))
     render 
   end
   def myEnableCallbackBool
     puts "BOOLEAN #{@params.inspect}"
     $cnt = $cnt + 1
     data = $cnt
       
         Rho::WebView.executeJavascript("showCounter5('" + data.to_s + "');")
    
   end
  def indexarray
     @msg = @params['msg']
     $cnt = 0
     Rho::Dummy.enableArray(2, url_for(:action => :myEnableCallbackArray))
     render 
   end
   def myEnableCallbackArray
     puts "ARRAY #{@params.inspect}"
     $cnt = $cnt + 1
     data = $cnt
       
         Rho::WebView.executeJavascript("showCounter6('" + data.to_s + "');")
