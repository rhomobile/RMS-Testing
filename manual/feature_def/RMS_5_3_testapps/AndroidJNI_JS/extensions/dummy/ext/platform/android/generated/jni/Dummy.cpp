...
const char* const CDummyBase::ENABLE_INT_TASK_CLASS = 
        "com.rho.dummy.DummyBase$enable_IntTask";

jclass CDummyBase::s_clsenable_IntTask = 0;
jmethodID CDummyBase::s_midenable_IntTask;

const char* const CDummyBase::ENABLE_STR_TASK_CLASS = 
        "com.rho.dummy.DummyBase$enable_strTask";

jclass CDummyBase::s_clsenable_strTask = 0;
jmethodID CDummyBase::s_midenable_strTask;

const char* const CDummyBase::ENABLE_BOOL_TASK_CLASS = 
        "com.rho.dummy.DummyBase$enable_boolTask";

jclass CDummyBase::s_clsenable_boolTask = 0;
jmethodID CDummyBase::s_midenable_boolTask;

const char* const CDummyBase::ENABLE_DOUBLE_TASK_CLASS = 
        "com.rho.dummy.DummyBase$enable_doubleTask";

jclass CDummyBase::s_clsenable_doubleTask = 0;
jmethodID CDummyBase::s_midenable_doubleTask;
...
JNIEnv* CDummyBase::jniInit(JNIEnv* env)
{
    ...
    if(!initialized)
    {
        ...
        s_clsenable_IntTask = loadClass(env, ENABLE_INT_TASK_CLASS);
        if (!s_clsenable_IntTask) return 0;
        s_midenable_IntTask = env->GetMethodID(s_clsenable_IntTask, "<init>",
                        "(Lcom/rho/dummy/IDummy;ILcom/rhomobile/rhodes/api/IMethodResult;)V");
        if(!s_midenable_IntTask)
        {
            LOG(FATAL) + "Failed to get constructor for java class " + ENABLE_INT_TASK_CLASS;
            return NULL;
        }

        s_clsenable_strTask = loadClass(env, ENABLE_STR_TASK_CLASS);
        if (!s_clsenable_strTask) return 0;
        s_midenable_strTask = env->GetMethodID(s_clsenable_strTask, "<init>",
                        "(Lcom/rho/dummy/IDummy;ILcom/rhomobile/rhodes/api/IMethodResult;)V");
        if(!s_midenable_strTask)
        {
            LOG(FATAL) + "Failed to get constructor for java class " + ENABLE_STR_TASK_CLASS;
            return NULL;
        }

        s_clsenable_boolTask = loadClass(env, ENABLE_BOOL_TASK_CLASS);
        if (!s_clsenable_boolTask) return 0;
        s_midenable_boolTask = env->GetMethodID(s_clsenable_boolTask, "<init>",
                        "(Lcom/rho/dummy/IDummy;ILcom/rhomobile/rhodes/api/IMethodResult;)V");
        if(!s_midenable_boolTask)
        {
            LOG(FATAL) + "Failed to get constructor for java class " + ENABLE_BOOL_TASK_CLASS;
            return NULL;
        }

        s_clsenable_doubleTask = loadClass(env, ENABLE_DOUBLE_TASK_CLASS);
        if (!s_clsenable_doubleTask) return 0;
        s_midenable_doubleTask = env->GetMethodID(s_clsenable_doubleTask, "<init>",
                        "(Lcom/rho/dummy/IDummy;ILcom/rhomobile/rhodes/api/IMethodResult;)V");
        if(!s_midenable_doubleTask)
        {
            LOG(FATAL) + "Failed to get constructor for java class " + ENABLE_DOUBLE_TASK_CLASS;
            return NULL;
        }
