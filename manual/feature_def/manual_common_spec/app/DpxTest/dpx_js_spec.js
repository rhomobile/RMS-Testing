describe('DPX Functionality Test', function() {
    ...
    it('Objective:\nDocument capture with template having barcode, ocr, omr, picture', function() {
        dispTestCaseRunning('Capture document from file.\n1. Run test.\n2. Wait for file browser (no more 10s).\n3. Select "/sdcard/templates/1024w_754h_Delivery Attempt Notification.yuv".\n4. Tap to scan.\n');
        dispExpectedResult('Captured document should get processed successfully.\nPress "Test Passed" after test completion.');
        ...
        runs(function() {
            ...
            dpxInstance.setCallback(function(dict) {
				...
				expect(dict['processedForm']['regions'][0]['processingMode']).toEqual(Rho.DPX.PM_BARCODE);
				expect(dict['processedForm']['regions'][1]['processingMode']).toEqual(Rho.DPX.PM_OCR);
				expect(dict['processedForm']['regions'][2]['processingMode']).toEqual(Rho.DPX.PM_OMR);
				expect(dict['processedForm']['regions'][3]['processingMode']).toEqual(Rho.DPX.PM_PICTURE);
            });
