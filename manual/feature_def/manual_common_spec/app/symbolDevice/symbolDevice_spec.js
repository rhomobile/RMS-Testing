describe('Symbol Device test cases : ', function(){
	describe('Non Power test cases : ', function(){
		if(isAnyWindowsFamilyPlatform()){
			it("Should caliberate successfully with out optional callback", function(){
			...
			it("Should caliberate successfully with optional callback and press 'esc' ", function(){
		        ...
		        spec.addExpectation('Test passed, only if calibration screen is brought foreground and by pressing esc button caliberation should be unsuccessful.');
		        spec.displayScenario();
		        spec.waitForButtonPressing("Run test");
		        runs(function(){
		        	var caliberateCB = function(data){
			...
			it("Should caliberate successfully with optional callback and press 'Ok' ", function(){
		        ...
		        spec.addExpectation('Test passed, only if calibration screen is brought foreground and by pressing Ok button after performing caliberation should trigger with success callback.');
		        spec.displayScenario();
		        spec.waitForButtonPressing("Run test");
		        runs(function(){
		        	var caliberateCB = function(data){
			...
			it("Should not trigger caliberate callback when caliberation done via settings", function(){
				var spec = new ManualSpec(jasmine, window.document);
		    	spec.addGoal("Should not trigger caliberate callback when caliberation done via settings");
		        spec.addStep("Launch the test application press run test button and put it in the background.");
		        spec.addStep("Navigate to settings -> stylus -> Caliberate.");
