...
function updatePrinterList(printers) {
    for (var i = 0; i < printers.length; i++) {
        var printerInstance = EB.PrinterZebra.getPrinterByID(printers[i]);
...
function runSearch(options, timeout) {
    ...
    function searchPrinterCallback(callbackValue) {
        ...
        if (callbackValue.status == EB.PrinterZebra.PRINTER_STATUS_SUCCESS) {
            if (printer_id && printer_id.length > 0) {
                SO.discovered.push(printer_id);
                SO.last_printer_id = printer_id;
                SO.last_printer = EB.PrinterZebra.getPrinterByID(printer_id);
            } else {
                checkSearch();
            }
        } else {
    ...
    function checkSearch() {
        ...
        if ((SO.curr > SO.total) || ((elapsed + last_el) > timeout)) {
            ...
            if (SO.curr <= 1) {
                EB.PrinterZebra.searchPrinters(options, searchPrinterCallback);
            } else {
                setTimeout(function() {
                    EB.PrinterZebra.searchPrinters(options, searchPrinterCallback);
                }, 200);
...
$(window).load(
    function() {
        var conn_types = [EB.PrinterZebra.CONNECTION_TYPE_BLUETOOTH, EB.PrinterZebra.CONNECTION_TYPE_ON_BOARD, EB.PrinterZebra.CONNECTION_TYPE_TCP];
