...
class WebviewTestController < Rho::RhoController
  ...
  def executeJavascriptOnTab1
    ...
    Rho::WebView.executeJavascript("setDomElementText('executeJavascript_ruby_test', 'Tab with index 1 - this text was inserted by executeJavascript from ruby')")

  end

  def currentUrlOfCurrentTab
    url = Rho::WebView.currentURL()
    Rho::WebView.executeJavascript("setDomElementText('currentURL_test', '" + url +  "')")
