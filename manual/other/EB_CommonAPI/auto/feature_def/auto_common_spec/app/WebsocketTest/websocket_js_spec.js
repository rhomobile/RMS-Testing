...
if (EB.System.platform == EB.System.PLATFORM_ANDROID) {
    ...
    describe('Websocket JS API', function() {
        ...
        it('Creates websocket', function() {
          ...
          ws.onopen = function() {
            cbCalled = true;
            EB.Log.info("Websocket connection estabilished.", "WEBSOCKET");
          };

          ws.onclose = function(event) {
            cbCalled = true;
            EB.Log.info('Closed websocket with code: ' + event.code + ' reason: ' + event.reason, "WEBSOCKET");
          };

          ws.onmessage = function(event) {
            cbCalled = true;              
            EB.Log.info("Websocket data received " + event.data, "WEBSOCKET");
            receivedMessage = event.data;
          };

          ws.onerror = function(error) {
            cbCalled = true;
            EB.Log.error("Error " + error.message, "WEBSOCKET");
          };
	...
	describe("Web socket js api test by bhakta", function(){
      ...
      var cbOnerrorCalled = false;
			...
			ws3.onclose = function(event) {
				...
				EB.Log.info("Websocket connection closed.", "WEBSOCKET");
			};
      ...
      var ws = new Websocket(srvURL);
			...
			ws.onerror = function(error) {
				EB.Log.info("Websocket error: " + error, "WEBSOCKET");
				cbOnerrorCalled = true;
			};
			ws.onmessage = function(event) {
				EB.Log.info("Websocket data received " + event.data, "WEBSOCKET");
			...
			runs(function(){
				EB.Log.info("Closing websocket", "WEBSOCKET");
        cbOnopenCalled = false;
				ws.close();
			});
			waitsFor(function(){
      ...
      var cbOnerrorCalled = false;
			...
			ws3.onclose = function(event) {
				EB.Log.info("Websocket onclose", "WEBSOCKET");
				cbOncloseCalled = true;
			};
			ws3.onerror = function(error) {
				EB.Log.info("Websocket onerror " + error, "WEBSOCKET");
				cbOnerrorCalled = true;
			};
