			...
			describe("getProperty and get properties with all combination for "+ camtype +": "+ camname, function() {
				...
				beforeEach(function() {
					...
					it("VT285-077 | call getDefault |" + camtype, function() {
						    ...
						    EB.Camera.setDefault(enumObject);
						    var defaultobj = EB.Camera.getDefault();
							expect(camtype).toEqual(defaultobj.getProperty('ID'));
					});

					it("VT285-077A | call Default |" + camtype, function() {

						    //EB.Camera.setDefaultID( enumObject.getId());
						   	//EB.Camera.setDefault( enumObject );
						    //var defaultobj = EB.Camera.getDefault();
							expect(camtype).toEqual(defaultobj.getProperty('ID'));
					});
					...
					it("VT299-090 | check default values of all writeable property |", function() {
						...
						runs(function() {
						    ...
						    {
							    expect(EB.Signature.previewHeight).toBeGreaterThan(0);
								expect(EB.Signature.previewLeft).toBeGreaterThan(0);
								expect(EB.Signature.previewTop).toBeGreaterThan(0);
								expect(EB.Signature.previewWidth).toBeGreaterThan(0);
						    }

						    if (isApplePlatform() || isAndroidPlatform())
						    {
							    expect(EB.Signature.saveToDeviceGallery).toEqual(true);
							    expect(enumObject.colorModel).toEqual('rgb');
						    }

						    if (isApplePlatform())
						    {
						    	expect(EB.Signature.enableEditing).toEqual(true);
						    }

						    if (isAndroidPlatform())
						    {
						    	expect(EB.Signature.flashMode).toEqual('off');
						    	expect(EB.Signature.useSystemViewfinder).toEqual(false);
						    }
					...
					(function(idx){
						...
						it(arrCamera[idx]['testName'], function() {
							...
							runs(function() {
								...
								EB.Camera.choosePicture(objProperty, camCallback);
							...
							runs(function() {
								...
								var data = EB.Camera.getProperties(objGetProperty);
...
describe("Enumerate Camera ", function() {
	...
	it("VT285-014: Enumerate Camera callback as function", function() {
		...
		runs(function() {
			EB.Camera.enumerate(enumCallback);
		});
		waitsFor(function(){
			return enumObjCount;
		});
	...
	it("VT285-016: Enumerate Camera with anonymous function as callback", function() {
		...
		runs(function() {
			EB.Camera.enumerate(function(obj){
				enumCallback(obj);
			});
		});
		waitsFor(function(){
