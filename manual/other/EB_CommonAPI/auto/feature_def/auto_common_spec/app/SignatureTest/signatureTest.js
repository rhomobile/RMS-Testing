...
{
	signatureTest.screenWidth = EB.System.screenWidth;
	signatureTest.screenHeight = EB.System.screenHeight;
	signatureTest.platform = EB.System.platform;
...
{
	EB.Instrumentation.delete_file(url);
};
signatureTest.doDrawLine = function(x1, y1, x2, y2)
{
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_DOWN, x1, y1);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_MOVE, x2, y2);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_UP, left, top);
};
signatureTest.doDrawBox = function(top, left, width, height)
{
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_DOWN, left, top);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_MOVE, left, top + height);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_MOVE, left + width, top + height);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_MOVE, left + width, top);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_MOVE, left, top);
	EB.Instrumentation.simulate_touch_event(signatureTest.MOUSE_UP, left, top);
};
...
describe('Signature', function() {
	describe('BMP File Capture specs', function() {
		...
		it('should capture an empty signature capture', function() {
			runs(function ()
			{
				...
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('should capture a file with the specified name', function() {
			...
			runs(function ()
			{
				...
				expect(EB.Instrumentation.file_exists(signatureTest.callbackImage.src)).toBe(true);
			});
		});
	...
	describe('Parameters that clear the signature area', function() {
		...
		it('clears the signature capture area when penColor is set after drawing', function () {
			runs(function ()
			{
				...
				signatureTest.drawNonSym();
				EB.Signature.setProperty({penColor: '#000000FF'});
				signatureTest.drawBox();
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('clears the signature capture area when bgColor is set after drawing', function () {
			runs(function ()
			{
				...
				signatureTest.drawNonSym();
				EB.Signature.setProperty({bgColor: '#FFFFFFFF'});
				signatureTest.drawBox();
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('clears the signature capture area when height is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawNonSym();
				EB.Signature.setProperty({height: 100});
				signatureTest.drawBox();
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('clears the signature capture area when width is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawNonSym();
				EB.Signature.setProperty({width: 100});
				signatureTest.drawBox();
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
	...
	describe('Parameters that shouldnt clear the signature area', function() {
		...
		it('doesnt clear the signature capture area when left is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawBox();
				EB.Signature.setProperty({left: 0});
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('doesnt clear the signature capture area when top is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawBox();
				EB.Signature.setProperty({top: 0});
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('doesnt clear the signature capture area when border is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawBox();
				EB.Signature.setProperty({border: false});
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
		...
		it('doesnt clear the signature capture area when imageFormat is changed', function () {
			runs(function ()
			{
				...
				signatureTest.drawBox();
				EB.Signature.setProperty({imageFormat: 'bmp'});
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
	...
	describe('Fullscreen Tests', function() {
		...
		it('Sends an \'ok\' status when fullscreen signature is captured using the capture function', function() {
			runs(function ()
			{
				...
				EB.Signature.capture(signatureTest.statusCallback);
			});
	...
	describe('Vector Tests', function() {
		...
		it('should get vectors for a vertical line', function() {
			runs(function ()
			{
				EB.Signature.setVectorCallback(signatureTest.vectorCallback);
				EB.Signature.show({
					left:0,
					top:0,
					width:100,
		...
		afterEach(function() {
			EB.Signature.setVectorCallback(null);
			EB.Signature.hide();
			EB.Signature.clear();
			//signatureTest.simulateNavigation();
		});
	...
	describe('Defaults Tests', function() {
		...
		it('Doesnt exception if take is given an empty parameter object', function(){
			runs(function ()
			{
				EB.Signature.show({}, signatureTest);
				EB.Signature.hide();
			});
		...
		it('Captures using default values if take is given an parameter object with only one parameter', function(){
			runs(function ()
			{
				EB.Signature.show({outputFormat:'dataUri'});
				signatureTest.drawDefaultBox();
				EB.Signature.capture(signatureTest.imageCompareCallback);
			});
