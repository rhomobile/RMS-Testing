describe('EB.SimulScan APIs Set Test', function () {
    ...
    var failCaptureDocument = function (expectedFailureReason) {
        ...
        runs(function () {
            EB.SimulScan.captureDocument(function (dict) {
                if (dict['callbackType'] === EB.SimulScan.FAILURE) {
    ...
    var describePropertySetTest = function (property, values, invalidValues) {
        ...
        describe('Set invalid ' + property, function () {
            for (var j = 0; j < invalidValues.length; ++j) {
                (function (value) {
                    it("EB.SimulScan['" + property + "'] = " + value, function () {
                        EB.SimulScan.template = 'file://' + encodeURI('/sdcard/simulscan/templates/Logistics Post.xml');
                        EB.SimulScan[property] = value;
                        runs(function () {
                            expect(EB.SimulScan[property]).toEqual(value);
                        });
    ...
    var simulscanInstance = EB.SimulScan;
    ...
    describe('getting property default value', function () {
        ...
        it('Should get flashMode default value', function () {
            expect(simulscanInstance.flashMode).toEqual(EB.SimulScan.FLASH_OFF);
        });
        ...
        it('Should get inputSource default value', function () {
            expect(simulscanInstance.inputSource).toEqual(EB.SimulScan.SOURCE_CAMERA);
        });
        ...
        it('Should get ocrLanguage default value', function () {
            expect(simulscanInstance.ocrLanguage).toEqual(EB.SimulScan.LANGUAGE_ENGLISH);
        });
    ...
    describe('Setting flashMode', function () {
        ...
        it('Should Set flashMode to FLASH_OFF using direct calling method', function () {
            simulscanInstance.flashMode = EB.SimulScan.FLASH_OFF;
            expect(simulscanInstance.flashMode).toEqual(EB.SimulScan.FLASH_OFF);
        });

        it('Should Set flashMode to off using setProperty calling method', function () {
            simulscanInstance.setProperty('flashMode', EB.SimulScan.FLASH_OFF);
            expect(simulscanInstance.getProperty('flashMode')).toEqual(EB.SimulScan.FLASH_OFF);
        });

        it('Should Set flashMode to off using setProperties calling method', function () {
            simulscanInstance.setProperties({'flashMode': EB.SimulScan.FLASH_OFF});
            var data = simulscanInstance.getProperties(['flashMode']);
            data = data['flashMode'];
            expect(data).toEqual(EB.SimulScan.FLASH_OFF);
        });

        it('Should Set flashMode to FLASH_ON using direct calling method', function () {
            simulscanInstance.flashMode = EB.SimulScan.FLASH_ON;
            expect(simulscanInstance.flashMode).toEqual(EB.SimulScan.FLASH_ON);
        });

        it('Should Set flashMode to on using setProperty calling method', function () {
            simulscanInstance.setProperty('flashMode', EB.SimulScan.FLASH_ON);
            expect(simulscanInstance.getProperty('flashMode')).toEqual(EB.SimulScan.FLASH_ON);
        });

        it('Should Set flashMode to on using setProperties calling method', function () {
            simulscanInstance.setProperties({'flashMode': EB.SimulScan.FLASH_ON});
            var data = simulscanInstance.getProperties(['flashMode']);
            data = data['flashMode'];
            expect(data).toEqual(EB.SimulScan.FLASH_ON);
        });
    });
    ...
    describe('Setting inputSource', function () {
        ...
        it('Should Set inputSource to SOURCE_CAMERA using direct calling method', function () {
            simulscanInstance.inputSource = EB.SimulScan.SOURCE_CAMERA;
            expect(simulscanInstance.inputSource).toEqual(EB.SimulScan.SOURCE_CAMERA);
        });

        it('Should Set inputSource to camera using setProperty calling method', function () {
            simulscanInstance.setProperty('inputSource', EB.SimulScan.SOURCE_CAMERA);
            expect(simulscanInstance.getProperty('inputSource')).toEqual(EB.SimulScan.SOURCE_CAMERA);
        });

        it('Should Set inputSource to camera using setProperties calling method', function () {
            simulscanInstance.setProperties({'inputSource': EB.SimulScan.SOURCE_CAMERA});
            var data = simulscanInstance.getProperties(['inputSource']);
            data = data['inputSource'];
            expect(data).toEqual(EB.SimulScan.SOURCE_CAMERA);
        });
    });
    ...
    describe('Getting version', function () {
        it('Should return version value as a string', function () {
            expect(EB.SimulScan.version).isNotEmptyString();
        });
    });
    ...
    it('Nonexistent template URI', function () {
        EB.SimulScan.template = 'file:///nonexistent.xml';
        failCaptureDocument('error: Can not read template.');
    });

    it('HTTP template URI', function () {
        EB.SimulScan.template = 'http://rhomobile-suite.s3.amazonaws.com/tmp/Logistics%20Post.xml';
        failCaptureDocument('error: Can not read template.');
    });
