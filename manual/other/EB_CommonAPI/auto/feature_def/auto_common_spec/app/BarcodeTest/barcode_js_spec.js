describe("Barcode JS API", function() {
    ...
    for (var j = 0;j<enumData.length;j++){
		...
		(function(enumObject,arrScanner){
			...
			if(EB.System.platform == "ANDROID" && scnname == "2D Imager")
			{
			...
			describe("Enable Scanner "+ scnid +": "+ scnname + scntype, function() {
				...
				it("Enable "+ scnid + scnname, function() {
					...
					runs(function() {
						enableflag = false;
						//EB.Log.info(JSON.stringify(scanObject), "PATRO Enable");
						enumObject.enable();
						setTimeout(function() {
						enableflag = true;
						}, 10000);
...
//								alert(enumObject);
//							    EB.Barcode.setDefault(enumObject);
//							    var defaultobj = EB.Barcode.getDefault();
...
//							    //enumObject.clearAllProperties();
//							    //EB.Barcode.setDefaultID( enumObject.getId() );
//							   // EB.Barcode.setDefault( enumObject );
//							   // var defaultobj = EB.Barcode.getDefault();
//								expect(scnid).toEqual(defaultobj.getProperty('ID'));
//						});
			...
			describe("Disable Scanner "+ scnid +": "+ scnname + scntype, function() {
				...
				it("Disable "+ scnid + scnname, function() {
					...
					runs(function() {
						//EB.Log.info(JSON.stringify(scanObject), "PATRO Disable");
						enumObject.disable();
						setTimeout(function() {
						disableflag = true;
						}, 10000);
