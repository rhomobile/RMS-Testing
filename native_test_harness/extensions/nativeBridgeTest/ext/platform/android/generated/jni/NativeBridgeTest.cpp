...
const char* const CNativeBridgeTestBase::TESTAPI_TASK_CLASS = 
        "com.rho.nativebridgetest.NativeBridgeTestSingletonBase$testApiTask";

jclass CNativeBridgeTestBase::s_clstestApiTask = 0;
jmethodID CNativeBridgeTestBase::s_midtestApiTask;
...
JNIEnv* CNativeBridgeTestBase::jniInit(JNIEnv* env)
{
    ...
    if(!initialized)
    {
        ...
        s_clstestApiTask = loadClass(env, TESTAPI_TASK_CLASS);
        if (!s_clstestApiTask) return 0;
        s_midtestApiTask = env->GetMethodID(s_clstestApiTask, "<init>",
                        "(Lcom/rho/nativebridgetest/INativeBridgeTestSingleton;Ljava/util/List;Ljava/util/Map;Ljava/util/Map;Ljava/util/List;Lcom/rhomobile/rhodes/api/IMethodResult;)V");
        if(!s_midtestApiTask)
        {
            LOG(FATAL) + "Failed to get constructor for java class " + TESTAPI_TASK_CLASS;
            return NULL;
        }
